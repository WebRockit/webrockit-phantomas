...
PKGNAME=webrockit-phantomas
# this prefix will be added to all files in the package
INSTALLDIR=/opt/phantomjs/collectoids/phantomas
...
PKGDEPENDS="webrockit-nodejs-bin"
...
# set fpm parameters if this rpm requires dependencies before install
FPMDEPENDS=""
if [ ! -z ${PKGDEPENDS} ]
then
    for pkgdep in ${PKGDEPENDS}
    do
        FPMDEPENDS=" -d ${pkgdep}"
   done
fi
...
fpm -s dir -t ${PKGTYPE} -n ${PKGNAME} -v ${VERSION} --iteration ${ITERATION} -a ${PKGARCH} ${FPMDEPENDS} ./* 
